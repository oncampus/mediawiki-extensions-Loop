				...
				if ( $tmp_lsi->getTocLevel() > $level &&  $tmp_lsi->getTocLevel() <= $level + $maxLevel) {
					if ( empty( $tocNumber ) || strpos ( $tmp_lsi->tocNumber, $tocNumber ) === 0 ) { # the root page's toc number must be inside the displayed toc number
						...
						if ( $title != null ) {
							...
							$escaped = preg_replace(array('/\&/','/\>/'),array('&amp;','&gt;'),$tmp_lsi->tocText);
							$xml .= '<loop_toc_list> <php_link_internal text-decoration="no-underline" href="article'. $tmp_lsi->article.'"><bold>'. $tmp_pageNumber .'</bold> '. $escaped . '</php_link_internal></loop_toc_list>';
