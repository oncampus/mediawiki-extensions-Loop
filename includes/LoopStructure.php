...
class LoopStructure {
	...
	public function renderAsSimpleStructure() {
		global $wgTitle;
		...
		foreach( $this->structureItems as $structureItem ) {
			...
			if( (int)$structureItem->article == $wgTitle->mArticleID ) {
		...
		foreach( $this->structureItems as $structureItem ) {
			...
			$text .= '<div class="loopstructure-listitem loopstructure-level-' . $structureItem->tocLevel . '">' . $link . '</div>';
		}
